...
import { promptBool, promptAmount, promptAddress, displayContentCell, waitForTransaction } from '../utils/uiUtils';
...
const changeAdminAction = async(provider:NetworkProvider, ui:UIProvider) => {
    ...
    const isContractDeployed = await provider.isContractDeployed(minterContract.address)
    if (!isContractDeployed)
        throw("Minter Contract is not deployed");
